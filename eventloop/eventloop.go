...
func (loop *EventLoop) schedule(call js.FunctionCall, repeating bool) js.Value {
	if fn, ok := js.AssertFunction(call.Argument(0)); ok {
...
func (loop *EventLoop) setTimeout(call js.FunctionCall) js.Value {
	return loop.schedule(call, false)
}

func (loop *EventLoop) setInterval(call js.FunctionCall) js.Value {
...
// The instance of js.Runtime that is passed to the function and any Values derived
// from it must not be used outside of the function. SetTimeout is
// safe to call inside or outside of the loop.
func (loop *EventLoop) SetTimeout(fn func(*js.Runtime), timeout time.Duration) *Timer {
...
// passed to ClearInterval. The instance of js.Runtime that is passed to the
...
// loop.
func (loop *EventLoop) SetInterval(fn func(*js.Runtime), timeout time.Duration) *Interval {
...
// The instance of js.Runtime that is passed to the function and any Values derived from it must not be used outside
// of the function.
...
func (loop *EventLoop) Run(fn func(*js.Runtime)) {
	loop.setRunning()
	fn(loop.vm)
	loop.run(false)
}
...
func (loop *EventLoop) RunOnLoop(fn func(*js.Runtime)) {
