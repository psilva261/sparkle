...
// In order to make use of this method you need an event loop such as the one in sparkle_nodejs (https://github.com/dop251/sparkle_nodejs)
// where it can be used like this:
//
//  loop := NewEventLoop()
//  loop.Start()
//  defer loop.Stop()
//  loop.RunOnLoop(func(vm *sparkle.Runtime) {
//		p, resolve, _ := vm.NewPromise()
//		vm.Set("p", p)
//      go func() {
//   		time.Sleep(500 * time.Millisecond)   // or perform any other blocking operation
//			loop.RunOnLoop(func(*sparkle.Runtime) { // resolve() must be called on the loop, cannot call it here
//				resolve(result)
//			})
//		}()
//  }
